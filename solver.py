...
from firedrake import grad, div, inner, dx, dot
...
Ra = fd.Constant(3.4e5) # value from paper
Pr = fd.Constant(0.71) # value from paper
K1 = fd.Constant(np.sqrt(0.71/3.4e5))
K2 = fd.Constant(1/np.sqrt((0.71*3.4e5)))
...
EvpT = fd.MixedElement([Ev, Ep, ET])
W = fd.FunctionSpace(mesh, EvpT)
...
def a(u,v):
    return (inner(grad(u)*u, v)*dx
        + inner(K1*grad(u), grad(v))*dx
        - T*inner(e_y, v)*dx) # - inner(p, div(v))*dx
def b(u,q):
    return inner(div(u), q)*dx
def c(u, T, S):
    return ( inner(u, grad(T))*S*dx + inner(K2*grad(T), grad(S))*dx - inner(T*e_y, u)*S*dx )
...
# Boundary conditions
bcs = [
    fd.DirichletBC(W.sub(0), fd.Constant((0, 0)), [1, 2, 3, 4]),
...
problem = fd.NonlinearVariationalProblem(F, w0, bcs,J)
